<template>
  <div class="home">
    ...
    <div v-if="searched && movies.length === 0" class="no-results">
      <p>Nothing found.</p>
    </div>
  ...
  setup () {
    ...
    const SearchMovies = async () => {
      if (search.value.trim() !== '') {
        try {
          const response = await fetch(`http://www.omdbapi.com/?apikey=${env.apikey}&s=${search.value}`);
            ...
            const filtered = await Promise.all(
              data.Search.map(async (movie) => {
                if (movie.Poster && movie.Poster !== 'N/A') {
                  const valid = await isImageAccessible(movie.Poster);
                  return valid ? movie : null;
                }
                return null;
              })
            );
            movies.value = filtered.filter(Boolean);
          } else {
            movies.value = [];
          }
        } catch (err) {
          console.error('Ошибка при получении данных с API:', err);
          movies.value = [];
        } finally {
          search.value = '';
        }
